...
export default class ChatHandler {
    ...
    openWebSocket() {
        ...
        this.socket.onerror = (error) => console.log('WebSocket error:', error.message);
    }
    ...
    readLookup(file, callback) {
        fetch(file)
            ...
            .then(data => {
                ...
                document.getElementById('nodeIDDisplay').textContent = this.nodeTable[this.myNodeID];
    ...
    sendMessage() {
        ...
        if (this.socket && this.socket.readyState === WebSocket.OPEN && Payload !== '') {
